 ...
 * @returns {Promise<movininTypes.Result<movininTypes.Booking>>}
 */
export const getBookingsByProperties = (propertyIds: string[], page: number, size: number): Promise<movininTypes.Result<movininTypes.Booking>> =>
  axiosInstance
    .post(
      `/api/bookings-by-properties/${page}/${size}/${UserService.getLanguage()}`,
      { propertyIds },
      { withCredentials: true }
    )
    .then((res) => res.data)
 ...
 * @returns {Promise<number>}
 */
export const cancelBooking = (id: string): Promise<number> =>
  axiosInstance
    .post(
