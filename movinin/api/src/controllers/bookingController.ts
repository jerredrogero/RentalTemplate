...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    if (env.VRBO_ENABLED) {
      try {
        // Placeholder for VRBO integration
        // In a real implementation, this would call the VRBO API to sync the calendar
        logger.info(`[booking.create] Booking ${booking._id} created - VRBO sync would happen here`)
      } catch (vrboErr) {
 ...
 * @returns {unknown}
 */
export const getBookingsByProperties = async (req: Request, res: Response) => {
  try {
    const { body } = req
    const { propertyIds } = body

    if (!propertyIds || !Array.isArray(propertyIds) || propertyIds.length === 0) {
      return res.status(400).send({ message: 'Property IDs are required' })
    }
    ...
    console.error(`[bookingController.getBookingsByProperties] ${err}`)
