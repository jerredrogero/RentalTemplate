      ...
      if (params.has('p')) {
        ...
        if (id && id !== '') {
          try {
            ...
            if (_property) {
              const propertyOwner = _property.agency || _property.owner
              if (_user.type === movininTypes.RecordType.Agency && _user._id !== propertyOwner?._id) {
                setLoading(false)
                setNoMatch(true)
                return
              }

              const _agency = propertyOwner ? {
                _id: propertyOwner._id as string,
                name: propertyOwner.fullName,
                image: propertyOwner.avatar,
              } : undefined
