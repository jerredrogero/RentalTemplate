...
export const LoginSocialGithub = ({ state = '', scope = 'repo,gist', client_id, client_secret, className = '', redirect_uri, allow_signup = false, isOnlyGetToken = false, isOnlyGetCode = false, children, onReject, onResolve, onLoginStart, }) => {
    useEffect(() => {
    ...
    const handlePostMessage = useCallback(async ({ type, code, provider }) => type === 'code' && provider === 'github' && code && getAccessToken(code), [getAccessToken]);
    ...
    const onLogin = useCallback(() => {
        ...
        window.open(oauthUrl, 'Github', 'menubar=no,location=no,resizable=no,scrollbars=no,status=no, width=' +
            width +
            ', height=' +
            height +
            ', top=' +
