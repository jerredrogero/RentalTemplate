...
export const LoginSocialInstagram = ({ state = '', client_id, client_secret, className, redirect_uri, fields = 'id,username,account_type,media_count', scope = 'user_profile,user_media', response_type = 'code', isOnlyGetCode = false, isOnlyGetToken = false, children, onReject, onResolve, onLoginStart, }) => {
    useEffect(() => {
    ...
    const onLogin = useCallback(() => {
        ...
        const oauthUrl = `${INSTAGRAM_URL}/oauth/authorize?response_type=${response_type}&client_id=${client_id}&scope=${scope}&state=${state + '_instagram'}&redirect_uri=${redirect_uri}`;
        const width = 450;
        const height = 730;
        const left = window.screen.width / 2 - width / 2;
        const top = window.screen.height / 2 - height / 2;
        window.open(oauthUrl, 'Instagram', 'menubar=no,location=no,resizable=no,scrollbars=no,status=no, width=' +
            width +
            ', height=' +
            height +
            ', top=' +
