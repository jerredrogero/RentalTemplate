...
const LoginSocialFacebook = ({ appId, scope = 'email,public_profile', state = true, xfbml = true, cookie = true, version = 'v2.7', language = 'en_EN', auth_type = '', className, onLoginStart, onReject, onResolve, redirect_uri, fieldsProfile = 'id,first_name,last_name,middle_name,name,name_format,picture,short_name,email,gender', response_type = 'code', return_scopes = true, isOnlyGetToken = false, children, }) => {
    ...
    const getMe = useCallback((authResponse) => {
        _window.FB.api('/me', { locale: language, fields: fieldsProfile }, (me) => {
            onResolve({
                provider: 'facebook',
    ...
    return (React.createElement("div", { className: className, onClick: loginFB }, children));
};
