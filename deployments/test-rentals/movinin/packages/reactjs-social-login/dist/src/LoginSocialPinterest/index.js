...
const PINTEREST_URL = 'https://www.pinterest.com/oauth';
const PINTEREST_URL_API = 'https://api.pinterest.com/v5';
// const PREVENT_CORS_URL: string = 'https://cors.bridged.cc';
export const LoginSocialPinterest = ({ state = '', scope = 'boards:read,pins:read,user_accounts:read', client_id, client_secret, className = '', redirect_uri, isOnlyGetCode = false, isOnlyGetToken = false, children, onLoginStart, onReject, onResolve, }) => {
    useEffect(() => {
    ...
    const getAccessToken = useCallback(async (code) => {
        if (isOnlyGetCode)
            onResolve({ provider: 'pinterest', data: { code } });
        else {
            var details = {
                code,
                redirect_uri,
    ...
    const onLogin = useCallback(() => {
        ...
        const oauthUrl = `${PINTEREST_URL}/?client_id=${client_id}&scope=${scope}&state=${state + '_pinterest'}&redirect_uri=${redirect_uri}&response_type=code`;
        const width = 450;
        const height = 730;
        const left = window.screen.width / 2 - width / 2;
        const top = window.screen.height / 2 - height / 2;
        window.open(oauthUrl, 'Pinterest', 'menubar=no,location=no,resizable=no,scrollbars=no,status=no, width=' +
            width +
            ', height=' +
            height +
            ', top=' +
