...
export const signin = async (req: Request, res: Response) => {
  ...
  try {
    console.log('Signin attempt:', emailFromBody);
    console.log('Request origin:', req.headers.origin);
    console.log('Request params:', req.params);

    if (!emailFromBody) {
      console.log('Email not found in request body');
    ...
    const user = await User.findOne({ email })
    console.log('User found:', !!user, user ? `type: ${user.type}` : 'no user');
    ...
    if (
      ...
      console.log('- Valid app type:', !![movininTypes.AppType.Frontend, movininTypes.AppType.Backend].includes(type));
      console.log('- Backend check:', !(type === movininTypes.AppType.Backend && user && user.type === movininTypes.UserType.User));
      console.log('- Frontend check:', !(type === movininTypes.AppType.Frontend && user && user.type !== movininTypes.UserType.User));
      return res.sendStatus(204)
    }
