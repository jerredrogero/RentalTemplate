...
export const create = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Ensure either agency or owner is provided
    if (!agency && !owner) {
      return res.status(400).send(i18n.t('AGENCY_OR_OWNER_REQUIRED'))
    }
...
export const getProperties = async (req: Request, res: Response) => {
  try {
    ...
    const agencies = body.agencies ? body.agencies.map((id) => new mongoose.Types.ObjectId(id)) : []
    const owners = body.owners ? body.owners.map((id) => new mongoose.Types.ObjectId(id)) : []
      ...
      $and: [
        {
          $or: [
            ...
            ...(owners.length > 0 ? [{ owner: { $in: owners } }] : []),
          ]
        },
        { location },
        { type: { $in: types } },
    ...
    const pipeline: any[] = [
      ...
      // Add a field to determine if we should use agency or owner data
      {
        $addFields: {
          agency: {
            $cond: {
...
export const getOwnerProperties = async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`[property.getOwnerProperties] ${i18n.t('DB_ERROR')} ${req.params.id}`, err)
    return res.status(400).send(i18n.t('DB_ERROR') + err)
  }
}

/**
 * Get company settings from configuration.
 *
 * @export
 * @async
 * @param {Request} req
...
export const getSettings = async (req: Request, res: Response) => {
  try {
    ...
    console.error(`[propertyController.getSettings] ${err}`);
    return res.status(400).send('Error getting settings');
  }
}
